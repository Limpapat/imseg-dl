...
def gen_empty_annf(root_dir:str, ann_dir:str, version:str, stamp:str, cats:list, imformat:str="png", image_size:int=512, gt_path:str=""):
    annf = {
        "info" : {
            "year" : stamp.split("-")[0],
            "version" : version,
    ...
    if len(gt_path) > 0:
        with open(gt_path, 'r') as f:
            gt_annf = json.loads(f.read())
        annf["images"] = gt_annf["images"]
    else:
        imformat = f"*.{imformat}"
